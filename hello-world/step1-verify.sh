...
[[ "$(cat fake-test-run.json | jq '.success' | grep -o -i -c '^true$')" = "1" ]] || (rm -f /root/my-app/src/AppFake*; rm -f /root/my-app/fake-test-run.json; false)
[[ "$(cat fake-test-run.json | jq '.success' | grep -o -i -c '^true$')" = "1" ]] && (rm -f /root/my-app/src/AppFake*; rm -f /root/my-app/fake-test-run.json; echo "done")
